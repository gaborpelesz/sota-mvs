...
def rescale_dataset(undistorted_path: str, new_width: int):
    """
    ...
    if len(undistorted_path_content) != 1:
        raise Exception(
            "Incorrect number of artifacts in provided path,"
    ...
    original_sparse_dir = os.path.join(
        undistorted_inner_path, "dslr_calibration_undistorted"
    )

    undistorted_rescaled_path = f"{undistorted_path}_{new_width}"
    undistorted_rescaled_inner_path = os.path.join(
        undistorted_rescaled_path, dataset_name
    )
    ...
    # RESCALE SPARSE
    cameras = colmapio.read_cameras_text(
        os.path.join(original_sparse_dir, "cameras.txt")
    )
    ...
    colmapio.write_cameras_text(
        cameras, os.path.join(rescaled_sparse_dir, "cameras.txt")
    )
    # images.txt and point3D.txt are the same so just copy them over
    shutil.copyfile(
        os.path.join(original_sparse_dir, "images.txt"),
        os.path.join(rescaled_sparse_dir, "images.txt"),
    )
    shutil.copyfile(
        os.path.join(original_sparse_dir, "points3D.txt"),
        os.path.join(rescaled_sparse_dir, "points3D.txt"),
    )
        ...
        for fname in fnames:
            ...
            if is_image(fpath):
                ...
                image_outpath = os.path.join(
                    rescaled_images_dir, os.path.relpath(fpath, original_images_dir)
                )
